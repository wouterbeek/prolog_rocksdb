...
typedef enum
{
   BLOB_ATOM, /* UTF-8 string as atom */
...
static unsigned alias_size {ALIAS_HASH_SIZE};
static alias_cell* alias_entries[ALIAS_HASH_SIZE];

static unsigned atom_hash(atom_t a)
{
...
static int save_rocks(atom_t aref, void* fd)
{
  dbref** refp = (dbref**)PL_blob_data(aref, nullptr, nullptr);
  dbref* ref = *refp;
  (void)fd;
...
static atom_t load_rocks(void* fd)
{
  (void)fd;
...
static int unify_rocks(term_t t, dbref* ref)
{
  if (ref->name) {
    if (!ref->symbol) {
      PlTerm tmp;
...
static dbref* symbol_dbref(atom_t symbol)
{
  void* data;
  size_t len;
  PL_blob_t* type;
  ...
  return (dbref*)nullptr;
}

static int get_rocks(term_t t, dbref** erp, int warn =TRUE)
{
  atom_t a;
  if (PL_get_atom(t, &a)) {
...
static int ok(const rocksdb::Status& status)
{
  if (status.ok()) {
    return TRUE;
  }
...
static void get_slice(term_t t, PlSlice& s, blob_type type)
{
  char* str;
  size_t len;
  switch(type) {
...
static int unify(term_t t, const rocksdb::Slice& s, blob_type type)
{
  switch(type) {
  case BLOB_ATOM:
  ...
  case BLOB_TERM:
    {
      PlTerm tmp;
      return (PL_recorded_external(s.data_, tmp) && PL_unify(tmp, t));
    }
  default:
    assert(0);
    return FALSE;
...
static int unify(term_t t, const std::string& s, blob_type type)
{
...
static int log_exception(rocksdb::Logger* logger)
{
  PlException ex {PL_exception(0)};
  rocksdb::Log(logger, "%s", (char*)ex);
  return false;
}
...
static int call_merger(const dbref* ref,
                       PlTermv av,
                       ...
                       rocksdb::Logger* logger)
{
  ...
  try {
    ...
    if (q.next_solution()) {
      ...
      rocksdb::Log(logger, "merger failed");
      return false;
    }
...
class PrologMergeOperator : public rocksdb::MergeOperator
{
  const dbref* ref;
public:
  PrologMergeOperator(const dbref* reference)
...
static void get_blob_type(PlTerm t, blob_type* key_type)
{
  atom_t a;
  if (PL_get_atom(t, &a))
  {
    if (ATOM_atom == a) {
      *key_type = BLOB_ATOM;
    } else if (ATOM_string == a) {
      *key_type = BLOB_STRING;
    } else if (ATOM_binary == a) {
      *key_type = BLOB_BINARY;
    } else if (ATOM_int32 == a) {
      *key_type = BLOB_INT32;
    } else if (ATOM_int64 == a) {
      *key_type = BLOB_INT64;
    } else if (ATOM_float == a) {
      *key_type = BLOB_FLOAT32;
    } else if (ATOM_double == a) {
      *key_type = BLOB_FLOAT64;
    } else if (ATOM_term == a) {
      *key_type = BLOB_TERM;
    } else {
...
PREDICATE(rocksdb_put, 3)
{
  ...
  ok(ref->db->Put(rocksdb::WriteOptions(), key, value));
  return TRUE;
}

PREDICATE(rocksdb_merge, 3)
{
  dbref* ref;
  PlSlice key, value;
  get_rocks(A1, &ref);
  ...
  get_slice(A2, key, ref->type.key);
  get_slice(A3, value, ref->type.value);
  ok(ref->db->Merge(rocksdb::WriteOptions(), key, value));
  return TRUE;
}

PREDICATE(rocksdb_get, 3)
{
  dbref* ref;
  PlSlice key;
  std::string value;
  get_rocks(A1, &ref);
  get_slice(A2, key, ref->type.key);
  return (ok(ref->db->Get(rocksdb::ReadOptions(), key, &value)) &&
          unify(A3, value, ref->type.value)
   );
}

PREDICATE(rocksdb_delete, 2)
{
  dbref* ref;
  PlSlice key;
  get_rocks(A1, &ref);
  get_slice(A2, key, ref->type.key);
  return ok(ref->db->Delete(rocksdb::WriteOptions(), key));
}
...
PREDICATE_NONDET(rocksdb_enum, 3)
{
  ...
  case PL_FIRST_CALL:
    ...
    state->it = state->ref->db->NewIterator(rocksdb::ReadOptions());
...
static void batch_operation(const dbref* ref,
...
PREDICATE(rocksdb_batch, 2)
{
  ...
  return ok(ref->db->Write(rocksdb::WriteOptions(), &batch));
}

static PlAtom ATOM_estimate_num_keys {"estimate_num_keys"};

PREDICATE(rocksdb_property, 3)
{
  dbref* ref;
  atom_t prop;
  get_rocks(A1, &ref);
